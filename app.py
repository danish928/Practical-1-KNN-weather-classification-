...
df["Weather"] = ["Sunny" if val == 0 else "Rainy" for val in y]

new_point = pd.DataFrame([[temp, hum, label_map[prediction]]],
                         columns=["Temperature", "Humidity", "Weather"])
df = pd.concat([df, new_point], ignore_index=True)

st.scatter_chart(df, x="Temperature", y="Humidity", color="Weather")
